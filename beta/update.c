...
// Funktion zum Installieren eines Debian-Pakets
int installPackage(const char *packageName) {
    char command[FILENAME_MAX_LENGTH + 15]; // +15 für zusätzliche Zeichen
    snprintf(command, sizeof(command), "pkg install -y %s.deb", packageName);
...
int main(int argc, char *argv[]) {
    ...
    while (fgets(url, URL_MAX_LENGTH, urlFile)) {
        ...
        // Konstruiere die vollständige URL des Pakets basierend auf dem ersten Argument
        char fullUrl[URL_MAX_LENGTH];
        if (strcmp(argv[1], "-si") == 0 || strcmp(argv[1], "-su") == 0 || strcmp(argv[1], "-sd") == 0) {
            char prefix = (argv[1][2] == 'i') ? 'i' : ((argv[1][2] == 'u') ? 'u' : 'd');
            snprintf(fullUrl, URL_MAX_LENGTH, "%s/pkg/%c/%s", url, prefix, packageName);
        } else {
        ...
        if (strcmp(argv[1], "-i") == 0 || strcmp(argv[1], "-u") == 0 || strcmp(argv[1], "-d") == 0) {
            ...
            if (downloadFile(fullUrl, filename)) {
                ...
                // Installiere das Paket und lösche die heruntergeladene Datei
            ...
            char searchFilename[FILENAME_MAX_LENGTH];
            sprintf(searchFilename, "%s.deb", packageName);

            if (downloadFile(fullUrl, searchFilename)) {
                printf("%s found under %s\n", searchFilename, url);
                remove(searchFilename); // Lösche die heruntergeladene Datei nach dem Suchen
                return 0;
            }
        } else {
